...
class DeepCFRSolver(policy.Policy):
  ...
  def __init__(self,
    ...
    if self._save_strategy_memories:
      if os.path.isdir(self._save_strategy_memories):
        self._strategy_memories_tfrecordpath = os.path.join(
  ...
  def _get_strategy_dataset(self):
    ...
    if self._strategy_memories_tfrecordpath:
      data = tf.data.TFRecordDataset(self._strategy_memories_tfrecordpath)
    else:
